...
//            T mock = proxyClass.newInstance();
            if(mock instanceof ClassProxy){
                ...
                Method setHandlerMethod = proxyClass.getMethod("setHandler", InvocationHandler.class);
                setHandlerMethod.invoke(mock, invocationHandler);
            }
  ...
  private InvocationHandlerAdapter getInvocationHandlerAdapter(Object mock) {
      ...
      try {
          Method getHandler = mock.getClass().getMethod("getHandler");
          if(getHandler != null)
            return (InvocationHandlerAdapter) getHandler.invoke(mock);
...
//                  if (lastDot != -1) names.add(clazz.getName().substring(0, lastDot));
                  ...
                  StringBuilder moxyClassName = new StringBuilder();

                  if(typeToMock.getPackage() != null)
                  {
                      moxyClassName.append(typeToMock.getPackage().getName());
                      if(moxyClassName.length() > 0)
                          moxyClassName.append(".");
                  }

                  int classnameCount = 0;
                  for (String name : names) {
                      if(classnameCount > 0)
                        moxyClassName.append("__");
                      moxyClassName.append(name);
                      classnameCount++;
                  }

                  moxyClassName.append("$Moxy");
//                  System.out.println("Moxy class: "+ moxyClassName.toString());

                  newClass = Class.forName(moxyClassName.toString());
                  System.out.println("Moxy class found ("+ newClass.getName() +") for string ("+ moxyClassName.toString() +")");
//                  System.out.println("Moxy class found! "+ newClass.getCanonicalName());
